...
const ROWS = 12;
const MULTIPLIERS = [0.5, 1, 3, 10, 3, 1, 0.5, 1, 3, 10, 3, 1, 0.5];
...
function Ball({ x, y, falling }: { x: number; y: number; falling: boolean }) {
  return (
    <motion.div
      ...
      transition={{ duration: 0.1, repeat: falling ? Infinity : 0 }}
    />
  );
}
...
function Slot({ multiplier, highlight }: { multiplier: number; highlight: boolean }) {
  const getColor = () => {
    if (multiplier >= 10) return "from-red-500 to-orange-500";
    if (multiplier >= 3) return "from-yellow-500 to-amber-500";
    if (multiplier >= 1) return "from-green-500 to-emerald-500";
    return "from-gray-500 to-gray-600";
  };

  return (
    <div
      className={`h-16 border-2 border-white/20 flex items-center justify-center text-sm font-bold rounded-b-lg ${
        highlight ? "ring-2 ring-white ring-offset-2 ring-offset-black" : ""
      }`}
    >
      <div className={`bg-gradient-to-r ${getColor()} text-white px-3 py-1 rounded-full`}>
        {multiplier}×
      </div>
    </div>
  );
...
export const Route = createFileRoute("/games/plinko")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Drop balls through pegs for multipliers up to 10×." },
      { property: "og:title", content: "Plinko — PoolCasino" },
      { property: "og:description", content: "Drop balls through pegs for multipliers." },
...
function PlinkoGame() {
  ...
  function dropBall() {
    ...
    let currentX = 50;
    let currentY = 0;
    let path: Array<{ x: number; y: number }> = [{ x: 50, y: 0 }];
    ...
    const dropInterval = setInterval(() => {
      ...
      if (currentY >= 80) {
        ...
        const safeSlot = Math.max(0, Math.min(MULTIPLIERS.length - 1, slotIndex));
    ...
    <Layout>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-2">Drop the ball and watch it bounce through pegs to land in multiplier slots.</p>
        </div>
          ...
          <Card className="bg-gradient-to-br from-sky-900/30 to-blue-900/30 border-sky-700/50 shadow-2xl">
            <CardContent className="p-6">
              <div className="relative h-96 bg-black/50 rounded-xl border-2 border-white/10 overflow-hidden">
                ...
                {/* Ball */}
                {dropping && <Ball x={ballPosition.x} y={ballPosition.y} falling={true} />}
                  ...
                  {(["low", "medium", "high"] as const).map((level) => (
                    ...
                    >
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {level === "low" ? "Safe payouts" : level === "medium" ? "Balanced" : "High risk"}
                      </p>
                    </button>
                  ))}
                </div>
