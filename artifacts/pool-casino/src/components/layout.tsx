...
export function Layout({ children }: { children: React.ReactNode }) {
      ...
      try {
        const [chatData, friendsData] = await Promise.all([
          fetch(`${BASE}api/chat/unread`, { credentials: "include" }).then(
            (r) => (r.ok ? r.json() : { unreadCount: 0 }),
          ),
          fetch(`${BASE}api/friends`, { credentials: "include" }).then((r) =>
  ...
  React.useEffect(() => {
    ...
    const fetchCount = async () => {
      try {
        const r = await fetch(`${BASE}api/admin/pending-count`, {
          credentials: "include",
        });
        if (r.ok) {
          const d = await r.json();
          setPendingAdminCount(d.count ?? 0);
        }
      } catch {}
    };
    fetchCount();
  ...
  const handleAppeal = async () => {
    ...
    try {
      await apiFetch("api/user/appeal", {
        method: "POST",
        body: JSON.stringify({ message: appealMsg }),
      });
      toast({
        title: "Appeal Submitted",
        description: "An admin will review your appeal soon.",
  ...
  const handleRequestMoney = async () => {
    ...
    try {
      const data = await apiFetch("api/money-request", {
        method: "POST",
        body: JSON.stringify({ amount, message: moneyMsg.trim() || undefined }),
      });
      toast({
        title: "Request Sent!",
  ...
  const navLinks = [
    ...
    ...(u?.isAdmin
      ? [
          {
            href: "/admin",
            label: (u as any)?.isOwner ? "Owner" : "Admin",
            icon: <ShieldAlert className="w-4 h-4" />,
            badge: pendingAdminCount || null,
          },
        ]
      : []),
  ];
  ...
  return (
    <div
      className={`bg-background text-foreground flex flex-col overflow-x-hidden ${isChat ? "h-screen overflow-hidden" : "min-h-screen"}`}
    >
      {/* Top Navbar */}
            ...
            <nav className="hidden md:flex space-x-1 items-center">
              ...
              <div ref={gamesDropdownRef} className="relative">
                ...
                >
                  ...
                  Games
                  <span
                    className={`text-xs transition-transform duration-200 ${gamesDropdownOpen ? "rotate-180" : ""}`}
                  >
                    ▾
                  </span>
                </button>
                  ...
                  {gamesDropdownOpen && (
                    ...
                    >
                      {[
                        ...
                        {
                          href: "/casinos",
                          label: "Player-Owned Casinos",
                          icon: <span className="text-sm">🏦</span>,
                          sub: "Own & operate casinos",
                        },
                      ].map((item) => (
                        <Link
                          key={item.href}
                ...
                {!isLoading && user && !isGuest ? (
                  <div className="flex items-center gap-2">
                    ...
                    <Link href="/profile">
                      <Button
                        variant="ghost"
                        size="icon"
                        className="rounded-full bg-white/5 border border-white/10 overflow-hidden p-0 w-8 h-8"
                      >
                        {user.avatarUrl ? (
                          <img
                            src={user.avatarUrl}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Link href="/register">
                      <Button
                        variant="default"
                        size="sm"
                        className="shadow-[0_0_15px_rgba(0,255,170,0.3)] gap-1.5"
                      >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Link href="/register">
                      <Button
                        variant="default"
                        className="shadow-[0_0_15px_rgba(0,255,170,0.3)]"
                      >
                        Sign up
                      </Button>
                    </Link>
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="px-4 pt-2 pb-4 space-y-1">
              ...
              {/* Games section */}
              <div className="pl-3 py-2 text-xs text-muted-foreground uppercase tracking-wider font-medium">
                Games
              </div>
              {[
                {
              ...
              {navLinks.slice(1).map((link) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {link.label}
                    {link.href === "/chat" && totalNotifs > 0 && (
                      <span className="bg-primary text-black text-[10px] font-bold rounded-full px-1.5">
                        {totalNotifs}
                      </span>
                    )}
                  </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {[10000, 50000, 100000, 500000].map((a) => (
                      <button
                        key={a}
                        onClick={() => setMoneyAmount(a.toString())}
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="text"
                    value={moneyMsg}
                    onChange={(e) => setMoneyMsg(e.target.value)}
              ...
              <Button
                ...
                className="w-full bg-primary hover:bg-primary/80 text-black font-bold gap-2"
              >
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-xl font-bold text-red-400 flex items-center gap-2">
                    <AlertTriangle className="w-5 h-5" /> Appeal Ban
                  </h2>
                  <p className="text-sm text-muted-foreground mt-1">
                    Explain why you believe this action was unjust. An admin
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground text-right">
                  {appealMsg.length}/2000
                </p>
              </div>
              <Button
                onClick={handleAppeal}
                disabled={appealPending || appealMsg.trim().length < 10}
                className="w-full bg-red-800 hover:bg-red-700 text-white font-bold gap-2"
              >
      ...
      <main
        className={`flex-1 w-full ${isChat ? "overflow-hidden min-h-0 flex flex-col" : "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"}`}
      >
        {children}
      </main>
        ...
        <footer className="border-t border-white/5 py-8 mt-auto">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <p className="text-xs text-muted-foreground text-center md:text-right max-w-md">
              This is a simulator using fake money. No real money gambling
              occurs on this site.
            </p>
            <a
